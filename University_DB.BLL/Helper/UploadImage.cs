...
namespace HospitalSystem.BLL.Helper
{
    public static class UploadImage
    {
		public static string UploadFile(string FolderName, IFormFile File)
		{
			...
			try
			{
				...
				// 1 ) Get Directory
				string FolderPath = Directory.GetCurrentDirectory() + "/wwwroot/ProfileImage/" + FolderName;
		...
		public static string RemoveFile(string FolderName, string fileName)
        {
            try
            {
                var directory = Path.Combine(Directory.GetCurrentDirectory(), "/wwwroot/Image/", FolderName, fileName);
